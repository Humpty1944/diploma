...
Java_engine_EngineUtil_isSimdSupported(JNIEnv *env, jclass clazz) {
#if defined(__arm__)
    using namespace cpu_features;
    ArmFeatures features = GetArmInfo().features;
    return features.neon ? JNI_TRUE : JNI_FALSE;
...
Java_engine_EngineUtil_reNice(JNIEnv *env, jclass clazz, jint pid, jint prio) {
    setpriority(PRIO_PROCESS, pid, prio);
}
extern "C"
JNIEXPORT jboolean JNICALL
Java_engine_EngineUtil_chmod(JNIEnv *env, jclass clazz, jstring exe_path) {
